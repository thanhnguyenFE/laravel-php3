...
<div class="text-white">
    <div class="fixed z-40 w-full text-white transition duration-500 navbar"
        ...
        <div class="mx-auto max-w-7xl">
            <div class="px-4 xl:px-0 flex items-center h-16 xl:h-20 gap-2">
                ...
                <nav class="ml-12 hidden xl:flex items-center gap-10 z-30">
                    <a class="{{ request()->routeIs('client.home') ? 'text-red-500' : 'text-white' }} hover:text-red-500"
                       href="{{ route('client.home') }}">Trang chủ</a>
                    <a class="{{ request()->routeIs('client.schedule') ? 'text-red-500' : 'text-white' }} hover:text-red-500"
                       href="{{ route('client.schedule') }}">Lịch chiếu</a>
                    <a class="{{ request()->is('news') ? 'text-red-500' : 'text-white' }} hover:text-red-500" href="#">Tin
                        tức</a>
                    <a class="{{ request()->is('promotions') ? 'text-red-500' : 'text-white' }} hover:text-red-500"
                       href="#">Khuyến mãi</a>
                    <a class="{{ request()->is('ticket-prices') ? 'text-red-500' : 'text-white' }} hover:text-red-500"
                       href="#">Giá vé</a>
                    <a class="{{ request()->is('film-festivals') ? 'text-red-500' : 'text-white' }} hover:text-red-500"
                       href="#">Liên hoan phim</a>
                    <a class="{{ request()->is('about') ? 'text-red-500' : 'text-white' }} hover:text-red-500" href="#">Giới
                        thiệu</a>
                </nav>
                @if(auth()->user())
                    <div class="ml-auto flex items-center gap-4">
                        <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown"
                                class="bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br text-white text-xs font-medium me-2 px-2.5 py-2 rounded dark:bg-gray-700 dark:text-red-400 border border-red-400 cursor-pointer">{{auth()->user()->name}}</button>
                        <div id="dropdown"
                             class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 w-[150px] border border-red-400">
                            <ul class="py-2 text-sm text-gray-700 " aria-labelledby="dropdownDefaultButton">
                                <li>
                                    <a href="{{route('client.profile')}}" class="block px-4 py-2 hover:bg-red-100">Trang
                                        cá nhân</a>
                                </li>
                                <li>
                                ...
                                <li>
                                    <a href="{{route('client.logout')}}" class="block px-4 py-2 hover:bg-red-100">Đăng
                                        xuất</a>
                                </li>
                            </ul>
                        </div>
                ...
                @else
                    ...
                    transition duration-300 border-white"
                                data-modal-target="modal-register" data-modal-toggle="modal-register"
                                data-type-modal="register" id="button-show-modal-register">
                            Đăng kí
                        </button>
                        <button class="inline-flex items-center justify-center rounded-full text-sm font-medium bg-gradient-to-r from-red-400 via-red-500
                         to-red-600 hover:bg-gradient-to-br text-white text-destructive-foreground h-10 px-8 py-2 hover:scale-105 transition duration-300"
                                data-modal-target="modal-login" data-modal-toggle="modal-login" data-type-modal="login"
                                id="button-show-modal-login">
                            Đăng nhập
                        </button>
                    </div>
                @endif
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative border border-red-600 rounded-lg shadow" style="background-color: rgb(16 20 27)">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                    ...
                    <button type="button"
                            class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                            data-modal-hide="modal-register">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                             viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                ...
                <div class="px-5 pb-5">
                    <form class="space-y-4" action="{{route('client.register')}}" method="post">
                        @csrf
                        <div>
                            <label for="name" class="block mb-2 text-sm font-medium text-white dark:text-white">Tên tài
                                khoản</label>
                        ...
                        <div>
                            <label for="password" class="block mb-2 text-sm font-medium text-white dark:text-white">Enter
                                the password</label>
                            <input type="text" name="enter_password" id="password" placeholder="Enter the password"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                                   required/>
                        </div>
                        <button type="submit"
    ...
    <div class="mx-auto p-8">
        ...
        <div class="mb-6 flex flex-wrap items-center justify-center gap-4 sm:gap-10">
            <div class="flex items-center gap-6">
                <img src="https://chieuphimquocgia.com.vn/_next/image?url=%2Fimages%2Ffacebook.png&w=32&q=75"
                     class="w-[30px] h-[30px]">
                <img src="https://chieuphimquocgia.com.vn/_next/image?url=%2Fimages%2Fzalo.webp&w=32&q=75"
                     class="w-[30px] h-[30px]">
                <img src="https://chieuphimquocgia.com.vn/_next/image?url=%2Fimages%2Fyoutube2.png&w=32&q=75"
                     class="w-[30px] h-[30px]">
            </div>
            <div class="flex gap-5 items-center">
                <img src="https://chieuphimquocgia.com.vn/_next/image?url=%2Fimages%2Fgoogleplay.png&w=256&q=75"
                     class="w-[140px] h-[38px]">
                <img src="https://chieuphimquocgia.com.vn/_next/image?url=%2Fimages%2Fappstore.png&w=256&q=75"
                     class="w-[140px] h-[38px]">
                <img src="https://chieuphimquocgia.com.vn/_next/image?url=%2Fimages%2Fcertification.png&w=256&q=75"
                     class="w-[140px] h-[38px]">
            </div>
        </div>
